classdef GPR_Stationary_SLFM < handle
    ...
    methods
        ...
        function [exitflag,exitinfo] = Optimize(obj,A0,Kernal0)
            ...
            % 上下限
            lb = 0.00000001 + zeros(1,Do*obj.Q+obj.Q*N_para);
            lb(1:(Do*obj.Q)) = -inf;
            ub = [];
            % 约束
    ...
    methods (Access = private)
        ...
        function K = Kf_Mat(obj,A,X,HyperPara2)
            ...
            HyperPara = [ones(size(HyperPara2,1),1),HyperPara2];
            B = GPR_Stationary_SLFM.Bi_mat(A); % B - [size(A,1),size(A,1),obj.Q]
            Kf = Kf_Mat0(obj,X,HyperPara); % Kf - [size(X,2),size(X,2),obj.Q]
            for i=1:obj.Q
                K = K + kron(B(:,:,i),Kf(:,:,i));
            end
